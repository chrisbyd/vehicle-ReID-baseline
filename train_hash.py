...
def main():
    ...
    logger = logging.getLogger("{}_{}_{}_train".format(args.target_names, args.arch, args.hash_bit_number))
    ...
    for epoch in range(args.start_epoch, args.max_epoch):
        train(epoch, model, criterion_xent, criterion_htri, criterion_quant ,optimizer, trainloader, use_gpu, logger= logger)
...
def train(epoch, model, criterion_xent, criterion_htri, criterion_quant, optimizer, trainloader, use_gpu, logger = None):
    xent_losses = AverageMeter()
    htri_losses = AverageMeter()
    qua_losses = AverageMeter()
    accs = AverageMeter()
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            logger.info('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'Htri {htri.val:.4f} ({htri.avg:.4f})\t'
                   'QuantLoss {quant.val:.4f} ({quant.avg:.4f})'
